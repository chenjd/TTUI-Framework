...
public class UIFontInspector : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		mType = (FontType)EditorGUILayout.EnumPopup("Font category", mType);
		GUILayout.Space(18f);
		...
		if (!mFont.isDynamic && mFont.bmFont.isValid)
		{
			if (mFont.atlas != null)
			{
				if (NGUIEditorTools.DrawHeader("Symbols and Emoticons"))
				{
					...
					if (symbols.Count == 0)
					{
						EditorGUILayout.HelpBox("Want to add an emoticon to your font? In the field above buildingId ':)', choose a sprite, then hit the Add button.", MessageType.Info);
					}
